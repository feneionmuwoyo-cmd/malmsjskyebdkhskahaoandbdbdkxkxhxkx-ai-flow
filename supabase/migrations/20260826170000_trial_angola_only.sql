-- The shared database keeps the trial flow for Angola only.
-- Global accounts must not receive, expire, or be blocked by trial credits.
...
begin
  ...
  set automation_paused = true, automation_paused_until = null
  where user_id in (
  ...
  insert into public.notifications (user_id, title, message, type, link)
  select p.user_id, 'Período de teste terminado',
  ...
  where coalesce(p.market, 'angola') = 'angola'
    ...
    and not exists (select 1 from public.notifications n where n.user_id = p.user_id and n.type = 'trial_expired');

...
export default function MessagesAreaChart({ userId }: { userId: string | undefined }) {
  ...
  useEffect(() => {
    ...
    const key = `muwoyo-message-chart-${userId}`;
    const saved = window.localStorage.getItem(key);
    if (saved === "7" || saved === "30" || saved === "90" || saved === "365") setRange(saved);
  }, [userId]);

  useEffect(() => {
    if (userId && typeof window !== "undefined") window.localStorage.setItem(`muwoyo-message-chart-${userId}`, range);
  }, [userId, range]);

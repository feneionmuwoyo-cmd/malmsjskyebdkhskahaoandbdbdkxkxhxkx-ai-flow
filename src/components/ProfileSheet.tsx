  ...
  const saveField = async (field: Field) => {
    ...
    if (field === "email") {
      ...
      if (!nextEmail || nextEmail === user.email?.toLowerCase()) {
        err = "Informe um email diferente do atual";
      } else {
        const { error } = await (supabase as any).from("profiles").update({ pending_email: nextEmail, email_verified: false }).eq("user_id", user.id);
        if (error) err = error.message;
        else {
          window.sessionStorage.setItem("muwoyo_pending_email", nextEmail);
          await supabase.functions.invoke("send-verification-code", { body: { email: nextEmail } });
          setEditing(null);
          setOpen(false);
          navigate("/confirmar-email", { replace: true, state: { email: nextEmail, emailChange: true } });
          setSaving(false);
          return;
        }
      }

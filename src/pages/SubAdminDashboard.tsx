...
export default function SubAdminDashboard() {
  ...
  useEffect(() => {
    ...
    const key = `muwoyo-subadmin-panel-${user.id}`;
  ...
  useEffect(() => {
    if (!user || typeof window === "undefined") return;
    window.localStorage.setItem(`muwoyo-subadmin-panel-${user.id}`, JSON.stringify({ search }));
  }, [user, search]);
  const load = async () => {
    if (!user) return;
    const { data } = await sb
  ...
  return (
    ...
    >
      ...
      <UserGrowthChart rows={rows} title="Crescimento dos meus usuários" />
                ...
                <tr key={u.user_id} className="border-b last:border-0">
                  ...
                  <td className="space-x-2 text-right">
                    ...
                    <Button
                      ...
                      variant="ghost"
                      onClick={() =>
                        window.confirm(`Tem certeza que deseja eliminar ${u.full_name || "este usuário"}?`) && call({ action: "deleteUser", userId: u.user_id })
                      }
                    >

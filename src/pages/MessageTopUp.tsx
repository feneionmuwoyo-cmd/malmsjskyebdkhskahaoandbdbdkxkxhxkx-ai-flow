...
type SetupRequest = { id: string; status: string; amount_kz: number; created_at?: string; confirmed_at?: string | null };
...
export default function MessageTopUp() {
  ...
  const requestSetup = async () => {
    ...
    const { data, error } = await supabase.rpc("request_setup_payment");
    ...
    >
      ...
      <div className="grid grid-cols-1 items-stretch gap-4">
        {packs.map((p) => (
          <Card
            key={p.id}
            className="h-full min-w-0 border-border/60 shadow-sm transition hover:shadow-lg"
          >
            <CardHeader>
